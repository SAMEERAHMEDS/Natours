...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  //automatically logging in when we sign up for any web application,
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // 4) Check if user changed paassword afer token was issued
...
exports.isLoggedIn = async (req, res, next) => {
  if (req.cookies.jwt) {
    try {
      ...
      // 3) Check if user changed paassword afer token was issued
      if (currentUser.changesPasswordAfter(decoded.iat)) {
        return next();
      }
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    //roles = ['admin', 'lead-guide'] only for them permission for others will return error
...
//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
